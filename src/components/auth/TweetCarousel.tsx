          ...
          <CarouselContent>
            {tweets.map((tweet, index) => (
              <CarouselItem key={index}>
                <div className="p-4 bg-gradient-to-br from-gray-900/90 to-black/90 rounded-lg border border-gray-800 backdrop-blur-sm">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-launch-cyan to-purple-500 flex items-center justify-center shadow-lg ring-2 ring-white/10">
                      <span className="text-white font-semibold text-base">
                        {tweet.author[0]}
                      </span>
                    </div>
                    <div>
                      <p className="text-white font-bold text-base">{tweet.author}</p>
                      <p className="text-gray-400 text-sm">{tweet.handle}</p>
                    </div>
                  </div>
                  <p className="text-white text-base leading-relaxed mb-4 font-medium">{tweet.content}</p>
                  <div className="flex items-center gap-3 text-gray-400">
                    <div className="flex items-center gap-1 group cursor-pointer">
                      <MessageCircle className="h-4 w-4 group-hover:text-launch-cyan transition-colors" />
